...
class NaViT(nn.Module):
    def __init__(self, *, image_size, patch_size, n_bboxs, n_classes, dim, depth, heads, mlp_dim, channels = 3, dim_head = 64, dropout = 0., emb_dropout = 0., token_dropout_prob = None, teacher_dim = 1024, init=False):
        super().__init__()
        ...
        # output to logits
        if teacher_dim == dim:
            print('teacher_dim == dim')
            self.proj = nn.Identity()
        else:
            self.proj = nn.Linear(dim, teacher_dim)
    ...
    def init_weights(self):
        ...
        # Initialize position embedding and cls token
        init.xavier_uniform_(self.pos_embedding)
        init.xavier_uniform_(self.cls_token)
